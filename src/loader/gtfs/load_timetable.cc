...
void load_timetable(loader_config const& config,
  ...
  auto trip_data =
      read_trips(tt, routes, service, shape_states, load(kTripsFile).data(),
                 config.bikes_allowed_default_, config.wheelchair_accessible_default_);
  ...
  auto const get_bikes_wheelchairs_seq =
      [&](std::basic_string<gtfs_trip_idx_t> const& trips, bitvec& seq_cache,
          bool trip::*member) -> bitvec const* {
    if (trips.size() == 1U) {
      return trip_data.get(trips.front()).*member
                 ? &kSingleTripTrue
                 : &kSingleTripFalse;
    } else {
      seq_cache.resize(0);
  ...
  auto const add_trip = [&](std::basic_string<gtfs_trip_idx_t> const& trips,
    ...
    expand_trip(
        ...
        assistance, [&](utc_trip&& s) {
          ...
          auto const* bikes_allowed_seq = get_bikes_wheelchairs_seq(
            s.trips_, bikes_allowed_seq_cache, &trip::bikes_allowed_);
          auto const* wheelchair_accessible_seq = get_bikes_wheelchairs_seq(
            s.trips_, wheelchair_accessible_seq_cache, &trip::wheelchair_accessible_);
          auto const it = route_services.find(
              route_key_ptr_t{clasz, stop_seq, bikes_allowed_seq, wheelchair_accessible_seq});
          if (it != end(route_services)) {
            ...
            route_services.emplace(
                route_key_t{clasz, *stop_seq, *bikes_allowed_seq, *wheelchair_accessible_seq,},
