...
// Timetable 1:
// Wheelchair access in T1, T3, T4, T6, but not in T0, T2, T5
// Connections with wheelchair accessibility are slower
// A -> C: 10:00 - 10:20 | T0 (no wheelchairs)
//         10:00 - 10:30 | T1 (wheelchair accessible)
//         11:00 - 11:30 | T2 (no wheelchairs)
// C -> D: 10:35 - 10:40 | T3 (wheelchair accessible)
//         10:45 - 10:50 | T4 (wheelchair accessible)
// A -> D: 10:00 - 10:40 | T0, T3 (no wheelchairs in T0)
//         10:00 - 10:40 | T1, T3 (wheelchair accessible)
// A -> F: 10:00 - 10:55 | T0, T3, T5 (no wheelchairs in T0, T5)
//         10:00 - 11:05 | T1, T4, T6 (wheelchair accessible)
...
constexpr auto const expected_A_C_no_wheelchairs =
    R"(
...
constexpr auto const expected_A_C_wheelchairs =
    R"(
...
constexpr auto const expected_A_D_no_wheelchairs =
    R"(
...
constexpr auto const expected_A_D_wheelchairs =
    R"(
[2019-05-01 08:00, 2019-05-01 08:40]
TRANSFERS: 1
     FROM: (A, A) [2019-05-01 08:00]
...
constexpr auto const expected_A_F_no_wheelchairs =
    R"(
...
constexpr auto const expected_A_F_wheelchairs =
    R"(
...
pareto_set<routing::journey> search(timetable const& tt,
                                    ...
                                    direction const dir,
                                    bool const require_wheelchairs_allowed = true) {
  return raptor_search(tt, rtt, dir == direction::kForward ? from : to,
                       dir == direction::kForward ? to : from, start_time, dir,
                       routing::all_clasz_allowed(), false, require_wheelchairs_allowed ? 2U : 0U);
}

}  // namespace

TEST(routing, wheelchair_accessible_test_1) {
  auto tt = timetable{};
  ...
  for (auto const dir : {direction::kForward, direction::kBackward}) {
    ...
    {  // A->C, without wheelchair
      auto const results =
          search(tt, nullptr, "A", "C", tt.date_range_, dir, false);
      EXPECT_EQ(expected_A_C_no_wheelchairs, results_to_str(results, tt));
    }

    {  // A->C, with wheelchair
      auto const results =
          search(tt, nullptr, "A", "C", tt.date_range_, dir, true);
      EXPECT_EQ(expected_A_C_wheelchairs, results_to_str(results, tt));
    }

    {  // C->D, without wheelchair
      auto const results =
    ...
    {  // A->D, without wheelchair
      auto const results =
          search(tt, nullptr, "A", "D", tt.date_range_, dir, false);
      EXPECT_EQ(expected_A_D_no_wheelchairs, results_to_str(results, tt));
    }

    {  // A->D, with wheelchair
      auto const results =
          search(tt, nullptr, "A", "D", tt.date_range_, dir, true);
      EXPECT_EQ(expected_A_D_wheelchairs, results_to_str(results, tt));
    }

    {  // A->F, without wheelchair
      auto const results =
          search(tt, nullptr, "A", "F", tt.date_range_, dir, false);
      EXPECT_EQ(expected_A_F_no_wheelchairs, results_to_str(results, tt));
    }

    {  // A->F, with wheelchair
      auto const results =
          search(tt, nullptr, "A", "F", tt.date_range_, dir, true);
      EXPECT_EQ(expected_A_F_wheelchairs, results_to_str(results, tt));
    }
  }
}

// Timetable 2:
// All trips wheelchair accessible except T1, blocks T1>T2 and T3>T4
// A -> D: 10:00 - 10:25 | T1, T2 (T1 not wheelchair accessible)
//         10:00 - 10:35 | T3, T4 (wheelchair accessible)
...
# trips.txt
route_id,service_id,trip_id,trip_headsign,block_id,bikes_allowed,wheelchair_accessible
R1,S1,T1,,1,2,2
R1,S1,T3,,2,2,1
R2,S1,T2,,1,2,1
R2,S1,T4,,2,2,1
...
constexpr auto const expected2_A_D_no_wheelchairs =
    R"(
...
constexpr auto const expected2_A_D_wheelchairs =
    R"(
...
constexpr auto const expected2_A_D_no_wheelchairs_rt =
    R"(
[2019-05-01 08:00, 2019-05-01 08:30]
TRANSFERS: 0
     FROM: (A, A) [2019-05-01 08:00]
...
constexpr auto const expected2_A_D_wheelchairs_rt =
    R"(
...
TEST(routing, wheelchair_accessible_test_2) {
  ...
  for (auto const dir : {direction::kForward, direction::kBackward}) {
    {  // A->D, without wheelchair
      auto const results =
          search(tt, nullptr, "A", "D", tt.date_range_, dir, false);
      EXPECT_EQ(expected2_A_D_no_wheelchairs, results_to_str(results, tt));
    }

    {  // A->D, with wheelchair
      auto const results =
          search(tt, nullptr, "A", "D", tt.date_range_, dir, true);
      EXPECT_EQ(expected2_A_D_wheelchairs, results_to_str(results, tt));
    }
  }
  ...
  for (auto const dir : {direction::kForward, direction::kBackward}) {
    {  // A->D, without wheelchair
      ...
      EXPECT_EQ(expected2_A_D_no_wheelchairs_rt, results_to_str(results, tt, &rtt));
    }

    {  // A->D, with wheelchair
      auto const results =
          search(tt, &rtt, "A", "D", tt.date_range_, dir, true);
      EXPECT_EQ(expected2_A_D_wheelchairs_rt, results_to_str(results, tt, &rtt));
    }
  }
}

// Timetable 3:
// All trips wheelchair accessible except T2, blocks T1>T2>T3 and T4>T5>T6
// A -> D: 10:00 - 10:30 | T1, T2, T3 (no wheelchairs in T2)
//         10:00 - 10:40 | T4, T5, T6 (wheelchair accessible)
...
constexpr auto const expected3_A_D_no_wheelchairs =
    R"(
[2019-05-01 08:00, 2019-05-01 08:30]
TRANSFERS: 0
     FROM: (A, A) [2019-05-01 08:00]
...
constexpr auto const expected3_A_D_wheelchairs =
    R"(
[2019-05-01 08:00, 2019-05-01 08:40]
TRANSFERS: 0
     FROM: (A, A) [2019-05-01 08:00]
...
TEST(routing, wheelchair_accessible_test_3) {
  auto tt = timetable{};
  ...
  for (auto const dir : {direction::kForward, direction::kBackward}) {
    {  // A->D, without wheelchair
      auto const results =
          search(tt, nullptr, "A", "D", tt.date_range_, dir, false);
      EXPECT_EQ(expected3_A_D_no_wheelchairs, results_to_str(results, tt));
    }

    {  // A->D, with wheelchair
      auto const results =
          search(tt, nullptr, "A", "D", tt.date_range_, dir, true);
      EXPECT_EQ(expected3_A_D_wheelchairs, results_to_str(results, tt));
    }
